...
def save_plot_as_html(fig, filepath, title, description):
    ...
    html = f'''<html><head><title>{title}</title></head><body>\n<h2>{title}</h2>\n<p>{description}</p>\n<img src="data:image/png;base64,{data}"/><br/></body></html>'''
...
def create_combined_report(models_data):
    ...
    for fmt in formats:
        pass
    # 优先插入PNG图片，否则插入HTML链接
    import os
    import warnings
    import matplotlib.image as mpimg
    import matplotlib.pyplot as plt
    def try_add_img(figs, titles, descriptions, png_path, html_path, title, desc):
        if os.path.exists(png_path):
            ...
            warnings.warn(f"{png_path} not found, {html_path} will be linked instead.")
            figs.append(None)
            titles.append(title)
            descriptions.append(desc + f' (See <a href="{os.path.basename(html_path)}">HTML</a>)')
    try_add_img(figs, titles, descriptions, 'Charts/all_format_size_before_after.png', 'Charts/all_format_size_before_after.html', 'Size Before/After Compression Comparison Across Formats', 'Comparison of file size before/after compression for each format (log scale, missing data marked)')
    try_add_img(figs, titles, descriptions, 'Charts/peak_memory_usage.png', 'Charts/peak_memory_usage.html', 'Peak Memory Usage', 'Peak memory usage for each model and format (log scale, missing data marked)')
    try_add_img(figs, titles, descriptions, 'Charts/all_format_size_before_after_linear.png', 'Charts/all_format_size_before_after_linear.html', 'Size Before/After Compression Comparison Across Formats (Linear, Large Height)', 'Comparison of file size before/after compression for each format (linear scale, large height, missing data marked)')
    # ... 其他图表 ...
    # ... existing code ...

def create_all_format_size_before_after(models_data):
    """合并Size Before/After Compression为一张分组堆叠柱状图，After区分Texture/Format数据"""
    formats = ['fbx', 'obj', 'glTF', 'glb']
    models = []
    face_counts = []
    texture_counts = []
    data_before = {fmt: [] for fmt in formats}
    data_after_texture = {fmt: [] for fmt in formats}
    data_after_format = {fmt: [] for fmt in formats}
    data_after_total = {fmt: [] for fmt in formats}
    for model_name, model_data in models_data.items():
        ...
        if has_data:
            ...
            for fmt in formats:
                if fmt in model_data['formats']:
                    before = model_data['formats'][fmt].get('size_before_mb', None)
                    after = model_data['formats'][fmt].get('size_after_mb', None)
                    tex = model_data['formats'][fmt].get('texture_size_mb', None)
                    data_before[fmt].append(before)
                    if after is not None and tex is not None:
                        data_after_texture[fmt].append(tex)
                        data_after_format[fmt].append(after - tex if after is not None and tex is not None else None)
                        data_after_total[fmt].append(after)
                    else:
                        data_after_texture[fmt].append(None)
                        data_after_format[fmt].append(None)
                        data_after_total[fmt].append(after)
                else:
                    data_before[fmt].append(None)
                    data_after_texture[fmt].append(None)
                    data_after_format[fmt].append(None)
                    data_after_total[fmt].append(None)
    # 过滤无数据模型
    ...
    for fmt in formats:
        ...
        data_after_texture[fmt] = [data_after_texture[fmt][i] for i in keep_indices]
        data_after_format[fmt] = [data_after_format[fmt][i] for i in keep_indices]
        data_after_total[fmt] = [data_after_total[fmt][i] for i in keep_indices]
    x = np.arange(len(models))
    width = 0.18
    fig, ax = plt.subplots(figsize=(max(10, len(models)*0.7), 8))
    ...
    for i, fmt in enumerate(formats):
        ...
        after_tex_vals = [v if v not in [None, 0] else 0 for v in data_after_texture[fmt]]
        after_fmt_vals = [v if v not in [None, 0] else 0 for v in data_after_format[fmt]]
        color_before = base_colors[i]
        color_after_tex = tuple(np.clip(np.array(base_colors[i]) + 0.3, 0, 1))
        color_after_fmt = tuple(np.clip(np.array(base_colors[i]) + 0.15, 0, 1))
        bars1 = ax.bar(x + offset, before_vals, width, label=f'{fmt} Before', color=color_before, zorder=2)
        bars2 = ax.bar(x + offset + width, after_fmt_vals, width, label=f'{fmt} After(Format data)', color=color_after_fmt, zorder=2)
        bars3 = ax.bar(x + offset + width, after_tex_vals, width, label=f'{fmt} After(Texture data)', color=color_after_tex, zorder=2, bottom=after_fmt_vals)
        # 标注before
        for bar, v in zip(bars1, data_before[fmt]):
            if v is None:
                ...
                ax.text(bar.get_x() + bar.get_width()/2., bar.get_height(), f'{v:.1f}', ha='center', va='bottom', fontsize=8, zorder=3)
        # 标注after总和（在最顶端）
        for j, (bar_fmt, bar_tex, total) in enumerate(zip(bars2, bars3, data_after_total[fmt])):
            if total is None:
                ax.text(bar_fmt.get_x() + bar_fmt.get_width()/2., 0.5, 'Missing', ha='center', va='bottom', fontsize=8, color='red', rotation=90, zorder=3)
            elif total not in [None, 0]:
                top = bar_fmt.get_height() + bar_tex.get_height()
                ax.text(bar_fmt.get_x() + bar_fmt.get_width()/2., top, f'{total:.1f}', ha='center', va='bottom', fontsize=8, zorder=3)
    all_values = []
    for fmt in formats:
        all_values += [v for v in data_before[fmt] if v not in [None, 0]]
        all_values += [v for v in data_after_total[fmt] if v not in [None, 0]]
    use_log = should_use_log_scale(all_values)
    ax.set_xlabel('Model (Face Count/Texture Count)', fontsize=12)
    ...
    for h, l in zip(handles, labels_):
        legend_dict[l] = h
    ax.legend(list(legend_dict.values()), list(legend_dict.keys()))
    ax.grid(True, alpha=0.3, which='both', zorder=1)
    if use_log:
        ax.set_yscale('log')
    plt.tight_layout()
...
def create_all_format_size_before_after_linear(models_data):
    ...
    x = np.arange(len(models))
    width = 0.18
    fig, ax = plt.subplots(figsize=(max(10, len(models)*0.7), 24))  # 高度大幅提升
    base_colors = plt.get_cmap('tab10').colors
    for i, fmt in enumerate(formats):
        offset = (i - 1.5) * width * 2
        before_vals = [v if v not in [None, 0] else 0 for v in data_before[fmt]]
    ...
    ax.set_title('Size Before/After Compression Comparison Across Formats (Linear, Large Height)', fontsize=16, fontweight='bold')
    ax.set_xticks(x)
    labels = [f'{m.split("_")[0]}\n({f}k/{t})' for m, f, t in zip(models, face_counts, texture_counts)]
    ax.set_xticklabels(labels, rotation=45, ha='right')
    handles, labels_ = ax.get_legend_handles_labels()
    ...
    ax.grid(True, alpha=0.3, which='both', zorder=1)
    plt.tight_layout()
    save_plot_as_html(fig, 'Charts/all_format_size_before_after_linear.html', 'Size Before/After Compression Comparison Across Formats (Linear, Large Height)', 'Comparison of file size before/after compression for each format (linear scale, large height, missing data marked)')
...
def main():
    ...
    print("\nGenerating all-format size before/after (linear, large height) report...")
